...
object fp3:
  ...
  def member(a: Int, xs: List[Int]): Boolean =
    ...
    xs match {
      case Nil => false
      case y :: ys => if y == a then true else member(a, ys)
    }
  ...
  def allEqual(xs: List[Int]): Boolean =
    ...
    xs match {
      case Nil => true
      case y :: ys => if ys.isEmpty then true else if y == ys.head then allEqual(ys) else false
    }
  ...
  def delete1[X](x: X, ys: List[X]): List[X] =
    ...
    ys match {
      case Nil => Nil
      case y :: ys => if y == x then delete1(x, ys) else y :: delete1(x, ys)
    }
  ...
  def removeDupes1[X](xs: List[X]): List[X] =
    ...
    xs match {
      case Nil => Nil
      case y :: ys => y :: removeDupes1(ys.dropWhile(_ == y))
    }
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    ...
    xs match {
      case Nil => Nil
      case y :: ys => {
        val (prefix, suffix) = xs.span(_ == y)
        (prefix.length, y) :: removeDupes2(suffix)
      }
    }
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    ...
    if n <= 0 then (Nil, xs)
      ...
      case Nil => (Nil, Nil)
      case y :: ys => {
        val (prefix, suffix) = splitAt(n - 1, ys)
        (y :: prefix, suffix)
      }
    }
  ...
  def allDistinct(xs: List[Int]): Boolean =
    ...
    xs match {
      case Nil => true
      case y :: ys => if member(y, ys) then false else allDistinct(ys)
    }
