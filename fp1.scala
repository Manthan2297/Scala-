...
object fp1:
  ...
  def maxList(xs: List[Int]): Int =
    ...
    case nonEmptyList => nonEmptyList.reduce((max, element) => if (element > max) element else max)
  }
  ...
  def maxTailAux(xs: List[Int], accumulator: Int): Int =
    ...
    xs match
      case Nil     => accumulator
      case y :: ys => maxTailAux(ys, if y > accumulator then y else accumulator)
